...
export default {
  name: 'SwipeCard',
  props: {
    // Add a prop to receive swipe direction from parent
    swipeDirection: {
      type: String,
      default: null
    }
  ...
  watch: {
    // Watch for changes to swipeDirection prop
    swipeDirection(direction) {
      if (direction) {
        this.programmaticSwipe(direction);
      }
    }
  },
  computed: {
    cardStyle() {
      ...
      if (this.isDraggingCard && (this.dragging || Math.abs(translateX) > 0)) {
        return {
          ...
          border: translateX > 20 ? `10px solid rgba(52, 199, 89, ${dragPercentage})` :
             translateX < -20 ? `10px solid rgba(255, 59, 48, ${dragPercentage})` : 
             'none'
        };
      } else {
  ...
  methods: {
    // Add new method for programmatic swipe
    programmaticSwipe(direction) {
      // Set the dragging flag to true to enable animations
      this.isDraggingCard = true;
      
      // Calculate the swipe distance based on direction
      const swipeDistance = direction === 'left' ? -this.maxDragDistance * 1.1 : this.maxDragDistance * 1.1;
      
      // Set current X position to trigger the swipe
      this.startX = 0;
      ...
      this.animateOffscreen(swipeDistance);
    },
    getClientX(e) {
